...
//anbhoir and muggu
uint64_t start_retired = 0;
int once = 0;
vector<vector<map<int, vector<pair<int, int>>>>> utility_BTB(
    NUM_TRACES,
    vector<map<int, vector<pair<int, int>>>>(6)
);
vector<int> partition_size = {16, 32, 64, 128, 256, 512};
vector<vector<int>> utility_BTB_hits(NUM_TRACES, vector<int>(partition_size.size(), 0));
vector<vector<int>> trace_file_utility(NUM_TRACES, vector<int>(partition_size.size(), 0));
...
vector<int> optimised_partitions;
...
void print_partition_stats(){
    for (int i = 0; i < NUM_TRACES; ++i) {
        ...
        for (int j = 0; j < partition_size.size(); ++j) {
            cout << utility_BTB_hits[i][j] << " ";
        }
        cout << endl;
    }
    cout << "Best Partition: ";
    for (int i = 0; i < NUM_TRACES; ++i) {
        cout << optimised_partitions[i] << " ";
    }
    cout << endl;
}

void total_print_partition_stats(){
    cout<<"check utility\n";
    for (int i = 0; i < NUM_TRACES; ++i) {
        for(int j=0; j < partition_size.size(); j++){
            cout << trace_file_utility[i][j] << " ";
        }
        cout << endl;
    }
}
vector<int> perform_partition(vector<vector<int>> hits){
    int n = NUM_TRACES;
    vector<int> best_partition(n);
    for (int i = 0; i < n; ++i) {
        int best_j = 0;
        for (int j = 1; j < 6; ++j)
            if (hits[i][j] > hits[i][best_j])
                best_j = j;
        best_partition[i] = partition_size[best_j];
    }
    vector<vector<int>> dp(n+1, vector<int>(1025, -1e9));
    vector<vector<int>> hitsum(n+1, vector<int>(1025, -1e9));
    vector<vector<int>> choice(n+1, vector<int>(1025, -1));
    ...
    for (int i = 0; i < n; ++i) {
        for (int s = 0; s <= 1024; ++s) {
            ...
            for (int j = 0; j < 6; ++j) {
                int p = partition_size[j];
                if (s + p <= 1024) {
                    int add = (p == best_partition[i] ? 1 : 0);
                    int hit_add = hits[i][j];
                    int new_best = dp[i][s] + add;
                    int new_hitsum = hitsum[i][s] + hit_add;

                    if (new_best > dp[i+1][s+p] || 
                        (new_best == dp[i+1][s+p] && new_hitsum > hitsum[i+1][s+p])) {
                        dp[i+1][s+p] = new_best;
                        hitsum[i+1][s+p] = new_hitsum;
                        choice[i+1][s+p] = j;
                    }
                }
            }
        ...
        int max_best_traces = dp[n][1024];
    int max_hits = hitsum[n][1024];
    ...
    for (int i = n; i >= 1; --i) {
        int j = choice[i][sum];
        allocation[i-1] = partition_size[j];
        sum -= partition_size[j];
    }

    for(int i =0 ; i< allocation.size(); i++ ){
        trace_file_utility[i][allocation[i]] = 1;
    }
...
//anbhoir and muggu 
int BTB_get_set(int ip, int size){
    ip = ip >> 2;
    return ip & ((1 << lg2(size)) - 1);
}

int BTB_get_tag(int ip, int size){
    ip = ip >> 2;
    return ip >> lg2(size);                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         
}

void lru_update(uint32_t set, uint32_t tag, int trace_number, int size)
{
    // update lru replacement state
    auto &vec = utility_BTB[trace_number][size][set];
    int way = 0;
    for(int i=0; i<vec.size(); i++){
        if(utility_BTB[trace_number][size][set][i].first == tag){
            way = i;
            break;
        }
    }
    for (uint32_t i=0; i<vec.size(); i++) {
        if (utility_BTB[trace_number][size][set][i].second < utility_BTB[trace_number][size][set][way].second) {
            utility_BTB[trace_number][size][set][i].second++;
        }
    }
    utility_BTB[trace_number][size][set][way].second = 0;
}

void evict_lru(int trace_number, int size, int set){
    // evict lru replacement state
    int way = 0, i = 0;
    for (i=0; i<4; i++) {
        if (utility_BTB[trace_number][size][set][i].second == 3) {
            way = i;
            break;
        }
    }
    if(i == 4){
        cerr << "No way to evict in utility BTB" << endl;
        assert(0);
    }
    utility_BTB[trace_number][size][set].erase(utility_BTB[trace_number][size][set].begin() + way);
}

void add_entry_in_utilityBTB(uint64_t ip, int trace_number){
    for(int i=0; i< partition_size.size(); i++){
        int set = BTB_get_set(ip, partition_size[i]);
        int tag = BTB_get_tag(ip, partition_size[i]);
        auto it = utility_BTB[trace_number][i].find(set);
        if(it != utility_BTB[trace_number][i].end()){
            auto& vec = it->second;
            bool found = false;

            for(auto& pair : vec){
                if(pair.first == tag){
                    utility_BTB_hits[trace_number][i]++;
                    lru_update(set, tag, trace_number, i);
                    found = true;
                    break;
                }
            }
            ...
            if(!found){
                if(vec.size() == 4){
                    evict_lru(trace_number, i, set);
                }
                for(int j=0; j<vec.size(); j++){
                    utility_BTB[trace_number][i][set][j].second++;
                }
                utility_BTB[trace_number][i][set].push_back({tag, 0});
            }
        } else {
            utility_BTB[trace_number][i][set] = {{tag, 0}};
        }
    }
}
...
{   
    ...
    cout<<trace_file<< " : "<<file_map[trace_file];
    cout<<" curr: "<<curr<<"\n";
    cout<<trace_files[curr]<<" : "<<file_map[trace_files[curr]] <<"\n";
...
void O3_CPU::read_from_trace()
{
    ...
    while (continue_reading) {
        ...
        if (knob_cloudsuite) {
            // cout<<"Reading from trace file: " << trace_string << endl;
		...
		//return; /*@Vasudha */
                ...
                arch_instr.trace_number = file_map[trace_file];
                ...
                {
                    add_entry_in_utilityBTB(arch_instr.ip, arch_instr.trace_number);
                }
...
//Neelu: Addition end
	...
	else
	  {
		...
		if (IFETCH_BUFFER.occupancy < IFETCH_BUFFER.SIZE) {
			...
			{
					uint32_t btb_set = BTB_get_set(arch_instr.ip >> 2, optimised_partitions[arch_instr.trace_number]);
                    for(int i = 0; i < arch_instr.trace_number; i++){
                        btb_set += optimised_partitions[i];
                    }
                    // if(warmup_complete[0]) cout<<"Trace file: "<<arch_instr.trace_number<<" check set: "<<btb_set<<endl;
                    //anbhoir and muggu
...
void O3_CPU::fill_btb(uint64_t trigger, uint64_t target, int trace_number)
{   //anbhoir and muggu
	uint32_t btb_set = BTB_get_set(trigger >> 2, optimised_partitions[trace_number]);
    for(int i = 0; i < trace_number; i++){
        btb_set += optimised_partitions[i];
    }
    // if(warmup_complete[0]) cout<<"Trace file: "<<trace_number<<" check set: "<<btb_set<<endl;
	  ...
	  for(uint32_t i=0; i<DECODE_BUFFER.SIZE; i++)
	  {
			...
			if(DECODE_BUFFER.entry[DECODE_BUFFER.head].btb_miss == 1 && DECODE_BUFFER.entry[DECODE_BUFFER.head].branch_mispredicted == 0)
		{
			...
			if(branch_type == BRANCH_DIRECT_JUMP || branch_type == BRANCH_DIRECT_CALL || (branch_type == BRANCH_CONDITIONAL))
			{
				...
				fill_btb(DECODE_BUFFER.entry[DECODE_BUFFER.head].ip, DECODE_BUFFER.entry[DECODE_BUFFER.head].branch_target, DECODE_BUFFER.entry[DECODE_BUFFER.head].trace_number);		
			}
		}
            ...
            if (ROB.entry[rob_index].branch_mispredicted)
	      	{
				...
				{
					fill_btb(ROB.entry[rob_index].ip, ROB.entry[rob_index].branch_target, ROB.entry[rob_index].trace_number);
				}
	     	}
    ...
    for (uint32_t n=0; n<RETIRE_WIDTH; n++) {
        ...
        miss_trace_file[trace_files[curr]].second++;
         ...
         if(warmup_complete[0]){
            ...
            if(num_retired - start_retired >= 2000000){
                if(once == 0){
                    once = 1;
                    start_retired = num_retired;
                }else{
                // cout<<current_cycles << " check cycles \n";
                optimised_partitions = perform_partition(utility_BTB_hits);
                start_retired = num_retired;
            ...
            if((num_retired - start_inst) >= random_inst){
                ...
                // random_inst = rand() % 10000 + 5000000;
