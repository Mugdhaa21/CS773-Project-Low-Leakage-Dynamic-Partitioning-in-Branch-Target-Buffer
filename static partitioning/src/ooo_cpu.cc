...
void O3_CPU::read_from_trace()
{
    ...
    while (continue_reading) {
        ...
        if (knob_cloudsuite) {
            ...
            if (trace_file == 0 || !fread(&current_cloudsuite_instr, instr_size, 1, trace_file)) {
                cout<<"code to check if it is hereh "<<endl;
...
//Neelu: Addition end
	...
	else
	  {
		...
		if (IFETCH_BUFFER.occupancy < IFETCH_BUFFER.SIZE) {
			...
			{       
                    //anbhoir and muggu static partitioning
					uint32_t btb_set = BTB.get_set(arch_instr.ip >> 2);
                    btb_set = arch_instr.trace_number * BTB_SET / NUM_TRACES + btb_set;
...
void O3_CPU::fill_btb(uint64_t trigger, uint64_t target, int trace_number)
{
    ...
    btb_set = trace_number * BTB_SET / NUM_TRACES + btb_set;
    //anbhoir and muggu
