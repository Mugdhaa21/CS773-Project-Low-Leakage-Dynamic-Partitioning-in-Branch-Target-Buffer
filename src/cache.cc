...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if((cache_type == IS_LLC)){
            if(MSHR.entry[mshr_index].cpu % 2 == 0){
                // set the 6th bit in addr from right to 0
                int mask = 1 << (lg2(NUM_SET)-1);
                set = set & mask;
            }else{
                // set the 6th bit in addr from right to 1
                set = set || (1 << (lg2(NUM_SET)-1));
            }
        }
...
if((cache_type == IS_L1I || cache_type == IS_L1D) && reads_ready.size() == 0)
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
        ...
        {   
            // if(cache_type == IS_LLC){
            //     cout<<"this is cpu"<<this->cpu<<endl;
            // }
            handle_fill();
            handle_writeback();
